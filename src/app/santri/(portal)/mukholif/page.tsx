                ...
                <div className="absolute z-10 w-full mt-2 bg-white border border-gray-100 rounded-xl shadow-xl max-h-60 overflow-y-auto">
                  {searchResults.map(santri => (
                    <button type="button" key={santri.id}
                      onClick={() => { onUpdate(data.id, 'selectedPelanggar', [santri]); setSearchQuery(""); setSearchResults([]); }}
          ...
          {data.selectedPelanggar.length > 0 && (
            <div className="flex flex-col gap-2 mt-2">
              {data.selectedPelanggar.map(p => (
                <div key={p.id} className="bg-red-50 text-red-700 border border-red-100 px-3 py-2.5 rounded-xl flex items-center justify-between gap-2 text-sm">
                  <div className="flex items-center gap-2">
                    <span className="w-6 h-6 rounded bg-white text-red-500 flex items-center justify-center shadow-sm">
                      <AlertTriangle size={14} />
                    </span>
                    <span className="font-bold">{p.nama}</span>
                  </div>
                  <button type="button" onClick={() => onUpdate(data.id, 'selectedPelanggar', [])}
                    className="p-1.5 hover:bg-red-100 rounded-lg transition-colors">
                    <X size={16} className="text-red-500 hover:text-red-900" />
                  </button>
                </div>
              ))}
              <div className="text-[11px] text-gray-500 italic mt-1 pb-1 leading-relaxed">
                * Satu form hanya untuk 1 orang pelanggar. Jika ada pelanggar lain di waktu yang sama, silakan gunakan tombol <b>"Tambah Form Laporan Lain"</b> di bawah.
              </div>
            </div>
          )}
        </div>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="flex flex-col sm:flex-row gap-3 pt-6 pb-2">
              ...
              >
                ...
                Tambah Form Untuk Pelanggar Berikutnya
              </button>
              <button
                type="submit"
